...
resource "oci_core_security_list" "FssSubnet" {
  count          = var.useExistingVcn ? 0 : 1
  compartment_id = var.compartment_ocid
  display_name   = "fss_security_list"
  vcn_id         = var.useExistingVcn ? var.custom_vcn[0] : oci_core_vcn.kubeflow_vcn.0.id
...
resource "oci_core_subnet" "file_system" {
  count                      = var.useExistingVcn ? 0 : 1
  cidr_block                 = var.fss_cidr
  display_name               = "file_system"
  compartment_id             = var.compartment_ocid
  vcn_id                     = var.useExistingVcn ? var.custom_vcn[0] : oci_core_vcn.kubeflow_vcn.0.id
  route_table_id             = oci_core_route_table.private[count.index].id
  security_list_ids          = [oci_core_security_list.FssSubnet.*.id[count.index]]
  dhcp_options_id            = oci_core_vcn.kubeflow_vcn[count.index].default_dhcp_options_id
  prohibit_public_ip_on_vnic = "true"
